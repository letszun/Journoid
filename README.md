...
도시와 여행 날짜를 직접 입력하고, 사진을 촬영 날짜와 시간대별 폴라로이드로 정리하는 개인 여행 기록 앱입니다.
...
- 브라우저에 여행 목록 저장
- 갤러리 이미지 여러 장 동시 불러오기
- JPEG EXIF 촬영 시각 우선 사용, 메타데이터가 없으면 파일 수정 시각 사용
- 날짜 및 새벽·아침·오후·저녁별 사진 분류
- 아이폰 기본 세로형 `3:4` 비율의 2D 폴라로이드 타임라인과 폴라로이드 아래 코멘트
- 흰색 전체 화면에서 드래그로 돌리는 3D 폴라로이드
- 폴라로이드를 눌러 사진 위에 낙서하고 80자 이내 코멘트 저장
- 두 장 단위 비동기 처리와 지연 저장으로 5장 이상 가져오기 반응성 개선
- 홈 화면 추가를 위한 standalone 웹 앱 설정
...
- `public/manifest.webmanifest`: 홈 화면 설치 정보
